...
// ComPtr<ID3D12Resource> m_instanceUploadBuffer[FrameCount];
// ComPtr<ID3D12Resource> m_instanceDefault;
// static const UINT MaxInstances = 500000;
// const UINT instBytes = MaxInstances * sizeof(InstanceDataCPU);
// uint8_t* m_instanceUploadMapped[FrameCount];    // We keep our mapping around so we can easily update the instances every frame.
...
//             // Place it at SRV heap slot 3 (t1)
//             // We have textures0, 1, and 2 already used.
//             m_device->CreateShaderResourceView(m_instanceDefault.Get(), &isrv, SrvCPU(3));
...
//             m_instanceDefault.Get(),
//             D3D12_RESOURCE_STATE_COPY_DEST,
//             D3D12_RESOURCE_STATE_GENERIC_READ);  // add PIXEL_SHADER_RESOURCE if PS also uses it
//         m_commandList->ResourceBarrier(1, &toSRV);
//         m_commandList->SetGraphicsRootDescriptorTable(5, SrvGPU(3));
//     }
